...
void transformCallback(const sensor_msgs::PointCloud2::ConstPtr& pc_msg_ptr)
{
    ...
    if(last_pc_ptr->size() < 10)
    {
        ...
        //*last_pc_ptr = *src_ptr;
        last_pc_ptr->clear();
        last_pc_ptr->resize(src_ptr->size());
        for(int i(0);i<src_ptr->size();++i)
        {
            last_pc_ptr->at(i) = src_ptr->at(i);
        }
        //normal_estimation.setInputCloud(last_pc_ptr);
        //normal_estimation.compute(*last_normal_ptr);
    ...
    icp.setInputSource(last_pc_ptr);
    if(src_ptr->size()<10) return;
    icp.setInputTarget(src_ptr);
    std::cout <<"4"<<std::endl;
    ...
    pcl::toROSMsg(*last_pc_ptr,pub_msg);
